...
- **2_water_source_analysis/**
  Contains notebooks focused on the relationship between irrigation expansion, CPIS siting, and water sources (dams, surface water, and groundwater). Analyses include targeting ratios near dams, elevation-based water transport feasibility, NDWI activity classification, Gaussian process groundwater interpolation, and anomaly detection. Outputs are stored in `Output/Analyze`.
